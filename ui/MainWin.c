...
#define SUBTITLE_LIST_TIME_FORMAT "%.3lf s"
...
void my_main_win_take_photo_cb (GtkButton *button, MyMainWin *self) {
  ...
  if (priv->pixbuf != NULL) {
    ...
    if (!g_access (temp, F_OK)) g_remove (temp);
    ...
    cairo_paint (cr);
    if (cairo_surface_write_to_png (surf, temp) == CAIRO_STATUS_SUCCESS) my_main_win_info_bar_show_info (
	self, 2, "Photo is Saved as %s", temp);
    else my_main_win_info_bar_show_info (self, 2,
					 "Photo is Failed to Saved as %s",
					 temp);
    cairo_destroy (cr);
...
GdkPixbuf* my_main_win_gen_subtitle_preview (MyMainWin *self, gchar *subtitle) {
  ...
  while (p != NULL) {
    ...
    layout = my_subtitle_parse_srt_text_line (
	cr, &font_color, gtk_font_chooser_get_font_family (priv->subtitle_font),
	12, temp);
...
guint64 my_main_win_add_subtitle (MyMainWin *self, gdouble start, gdouble end,
  ...
  my_trace_bar_add_obj_range (priv->trace_bar,
			      GUINT_TO_POINTER(priv->subtitle_index), start,
			      end, describe, &color);
  g_free (describe);
  ...
  gtk_list_store_set (priv->subtitle_list, &iter, col_index,
		      priv->subtitle_index, col_start, start, col_end, end,
		      col_text, subtitle, -1);
...
void add_subtitle_cb (GtkButton *button, MyMainWin *self) {
  ...
  gtk_spin_button_set_value (
      priv->subtitle_last_time,
      gtk_spin_button_get_value (priv->default_subtitle_last_time));
  gst_element_set_state (priv->pipeline, priv->cur_state);
}

void subtitle_auto_pause (GtkEntry *entry, gchar *new_text,
...
void subtitle_list_set_active (GtkCellRendererToggle *cell_renderer,
			       gchar *path, MyMainWin *self) {
  g_print ("set active\n");
}
...
void subtitle_list_set_start (GtkCellRendererText *renderer, gchar *path,
  ...
  s = gtk_adjustment_get_value(priv->subtitle_list_start_adj);
...
void subtitle_list_set_end (GtkCellRendererText *renderer, gchar *path,
  ...
  gtk_tree_model_get_iter (priv->subtitle_list, &iter, tpath);
  gtk_tree_path_free (tpath);
  e = gtk_adjustment_get_value(priv->subtitle_list_end_adj);
...
void subtitle_list_sync_data (MyMainWin *self, gpointer index) {
  ...
  gtk_list_store_set (priv->subtitle_list, &iter, col_start, s, col_start_text,
		      s_text, col_end, e, col_end_text, e_text, col_preview,
		      pixbuf, col_text, data->subtitle, -1);
  g_free (s_text);
  g_free (e_text);
  gdk_pixbuf_unref (pixbuf);
}
...
void subtitle_edit (MyMainWin *self) {
  ...
  while (gtk_tree_model_get_iter (priv->subtitle_list_edit_preview, &iter, path)) {
    gtk_tree_model_get (priv->subtitle_list_edit_preview, &iter,
			col_edit_preview_index, &index,
			col_edit_preview_beforce, &text, -1);
    ...
    if (g_strstr_len (format, -1, "%d")) {
      temp = g_strdup_printf ("%02d", g_date_time_get_day_of_month (datetime));
    ...
    gtk_list_store_set (priv->subtitle_list_edit_preview, &iter,
			col_edit_preview_after, new_text->str, -1);
...
void subtitle_list_edit_cb (GtkButton *button, MyMainWin *self) {
  ...
  while (gtk_tree_model_get_iter (priv->subtitle_list_edit_preview, &iter, path)) {
    gtk_tree_model_get (priv->subtitle_list_edit_preview, &iter,
			col_edit_preview_index, &index, col_edit_preview_after,
			&after, -1);
    data = g_hash_table_lookup (priv->subtitle_table, GUINT_TO_POINTER(index));
    g_free (data->subtitle);
    data->subtitle = after;
...
void subtitle_list_save_cb (GtkButton *button, MyMainWin *self) {
  ...
  if (respon == GTK_RESPONSE_OK) {
	...
	my_trace_bar_get_obj_range (priv->trace_bar,
				    GUINT_TO_POINTER(data->index), &start,
				    &end);
	my_subtitle_add_subtitle (subtitle, data->subtitle, start, end,
				  Subtitle_Format_None);
	if (key->next != NULL) key = key->next;
	else break;
      }
      g_list_free (key);
...
void subtitle_list_find_next_cb (GtkButton *button, MyMainWin *self) {
  ...
  switch (count) {
	...
	if (list->next == NULL) break;
	list = list->next;
	if (gtk_tree_path_compare (list->data, path) > 0) path = list->data;
      }
      break;
  }
  ...
  if (list != NULL) g_list_free_full (list, gtk_tree_path_free);
...
void subtitle_list_selection_cb (GtkTreeSelection *treeselection,
  ...
  if (count > 1) gtk_widget_set_sensitive (priv->subtitle_list_edit, TRUE);
  else gtk_widget_set_sensitive (priv->subtitle_list_edit, FALSE);
}
...
//update the subtitle_list in the treeview
  ...
  gtk_list_store_set (priv->subtitle_list, &iter, col_start, start,
		      col_start_text, start_text, col_end, end, col_end_text,
		      end_text, -1);
  g_free (start_text);
  g_free (end_text);

  if (priv->selected_index != index) return;
...
gboolean info_bar_close_cb (MyMainWin *self) {
  MyMainWinPrivate *priv = my_main_win_get_instance_private (self);
  priv->info_timeout--;
  if (priv->info_timeout != 0) return G_SOURCE_CONTINUE;
  gtk_info_bar_set_revealed (priv->info_bar, FALSE);
  return G_SOURCE_REMOVE;
}

void my_main_win_info_bar_show_info (MyMainWin *self, guchar timeout,
				     gchar *fmt, ...) {
  MyMainWinPrivate *priv = my_main_win_get_instance_private (self);
  va_list ap;
  va_start(ap, fmt);
  ...
  if (priv->info_timeout == 0) g_timeout_add (1000, info_bar_close_cb, self);
...
gpointer subtitle_normal_color[][4] = { { "red", 255, 0, 0 }, { "green", 0, 255,
    0 }, { "blue", 0, 0, 255 }, { "yellow", 255, 255, 0 }, { "purple", 255, 0,
    255 }, { "cyan", 0, 255, 255 }, { "white", 255, 255, 255 },
    { NULL, 0, 0, 0 },
    NULL, };
...
//draw the subtitle
  ...
  if (list->data != NULL) {
    ...
    //draw all subtitles
    while (n_subtitle > 0) {
      text_layout = my_subtitle_parse_srt_text_line (
	  cr, &fill_color,
	  gtk_font_chooser_get_font_family (priv->subtitle_font),
	  gtk_font_chooser_get_font_size (priv->subtitle_font), list_row->data);
...
void gst_bus_message_cb (GstBus *bus, GstMessage *message, MyMainWin *self) {
  ...
  if (s != NULL && gst_structure_has_field (s, "pixbuf")) {
	...
	&& gtk_toggle_button_get_active (priv->mul_photo)) {
      ...
      my_main_win_info_bar_show_info (self, 2, "Photo is Saved as %s", temp);
    }
...
static void *special_char_table[][12] = { { "General", 0x0021, 0x007f, 0x2600,
    ...
    0x3300, 0x33FF, NULL }, { "Arrow", 0x2190, 0x21ff, 0x27f0, 0x27ff, 0x2900,
    0x297f, NULL }, { "Math", 0x00D7, 0x00D7, 0x00F7, 0x00F7, 0x2200, 0x22ff,
    0x27c0, 0x27EF, 0x2980, 0x29ff, NULL },			//
    { "Music", 0x1D000, 0x1D0FF, 0x1D200, 0x1D24F, NULL }, { "Currency", 0x00A2,
	0x00A5, 0x20a0, 0x20cf, NULL }, { "Number", 0x2460, 0x24FF, NULL }, {
	"PinYin", 0x00A0, 0x00FF, 0x0100, 0x017F, 0x0180, 0x024F, NULL },//0x3100,0x312F,0x31A0,0x31BF,NULL},
...
void char_stack_init (MyMainWin *self) {
  ...
  while (special_char_table[i] != NULL) {
    ...
    gtk_stack_add_titled (priv->char_stack, scrolled, group_name, group_name);
    x = 0, y = 0;
	...
	g_signal_connect(button, "clicked", subtitle_add_special_char_cb, self);
...
static void my_main_win_class_init (MyMainWinClass *klass) {
  ...
  gtk_widget_class_bind_template_callback(klass, content_button_press_event_cb);
  ...
  gtk_widget_class_bind_template_child_private(klass, MyMainWin, info_message);
  ...
  gtk_widget_class_bind_template_child_private(klass, MyMainWin,
					       pop_menu_start);
  gtk_widget_class_bind_template_child_private(klass, MyMainWin, pop_menu_end);
  gtk_widget_class_bind_template_child_private(klass, MyMainWin,
					       pop_menu_subtitle);
  gtk_widget_class_bind_template_child_private(klass, MyMainWin,
					       pop_menu_label);
  gtk_widget_class_bind_template_child_private(klass, MyMainWin,
					       subtitle_color);
  gtk_widget_class_bind_template_child_private(klass, MyMainWin, subtitle_font);
  gtk_widget_class_bind_template_child_private(klass, MyMainWin,
  ...
  gtk_widget_class_bind_template_child_private(klass, MyMainWin, subtitle_list);
  gtk_widget_class_bind_template_child_private(klass, MyMainWin,
					       subtitle_treeview);
  gtk_widget_class_bind_template_child_private(klass, MyMainWin,
					       pop_label_preview);
  ...
  gtk_widget_class_bind_template_child_private(klass, MyMainWin, find_entry);
  gtk_widget_class_bind_template_child_private(klass, MyMainWin,
...
MyMainWin* my_main_win_new (gchar *title, gchar *uri, gboolean play) {
  ...
  if (title != NULL) gtk_window_set_title (m, title);
  if (uri != NULL) g_object_set (priv->src, "uri", uri, NULL);
  if (play) {
